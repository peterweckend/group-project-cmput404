...
WITH posts AS
    ...
    SELECT id FROM API_post WHERE 
        (author_id in  
            ...
            AND 
            (privacy_setting = 3 OR privacy_setting = 4)) 
            -- should be able to just add an OR (privacy setting = 5 and the two host infos match)
        OR author_id = %s OR  privacy_setting = 6)
    -- >>>>>>>>>> -------------------------------- <<<<<<<<<<
